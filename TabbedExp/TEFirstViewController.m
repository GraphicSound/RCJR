...
#define kLatestPosts [NSURL URLWithString:@"http://118.228.173.165/love/?json=get_category_posts&slug=shows"] //2
...
- (void)viewDidLoad
{
    ...
    //self.tabBarController.translucent= 0;
    
    //启动时先进行网络监测
    Reachability *reach = [Reachability reachabilityForInternetConnection];
    NetworkStatus status = [reach currentReachabilityStatus];
    NSString *currentNetwork = [self stringFromStatus:status];
    NSLog(@"%@", currentNetwork);
    
    pCount = 0;
    self.title = @"若诚就扰";//为什么不是self.navigationController.title？
    self.navigationController.tabBarItem.title = @"热门视频";
    //self.navigationItem.rightBarButtonItem = self.editButtonItem;//这个只是把按钮摆上去
...
//    self.tableView.tableFooterView = footer;
    ...
    {
        self.view.backgroundColor = [UIColor colorWithPatternImage:[UIImage imageNamed:@"view_bg_long"]];
    } else
    {
...
- (NSString *)stringFromStatus:(NetworkStatus) status {
    ...
    switch(status) {
        ...
        case ReachableViaWWAN:
            string = @"Reachable via WWAN";//表示3G
            break;
        default:
            string = @"Unknown";
            break;
...
- (void)fetchedData:(NSData *)responseData {
    ...
    for (int i=0; i<latestPosts.count; i++) {
        ...
        if ([oneContent rangeOfString:string1].location == NSNotFound) {
            ...
            NSString *urlString1 = [oneContent substringToIndex:range1.location+13];
            NSLog(@"%@", urlString1);
